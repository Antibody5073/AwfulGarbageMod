...
namespace AwfulGarbageMod.NPCs.BossUnrealRework.KingSlime
{
    ...
    public class KingSlimeShadow : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            NPC.lifeMax = 9999; // The amount of health that this npc has
        ...
        public override void AI()
        {
            ...
            if (bossPhase == 0)
            {
                ...
                atkUseCounter = 0;
                atkDelay = 0;
                NPC.GetGlobalNPC<GlobalEnemyBossInfo>().attack = 2;
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            if (true)
            {
                ...
                for (int k = 0; k < NPC.oldPos.Length; k++)
                {
                    ...
                    Color color = NPC.GetAlpha(Color.Black) * (float)(((float)(NPC.oldPos.Length - k) / (float)NPC.oldPos.Length) / 6);
            ...
            Vector2 drawOrigin2 = NPC.frame.Size() / 2;
            Vector2 drawPos2 = NPC.position - screenPos + new Vector2(NPC.width / 2, NPC.height / 2) + new Vector2(0, NPC.gfxOffY - 4); //.RotatedBy(NPC.rotation);
            Color color2 = NPC.GetAlpha(Color.Black) * 0.33f;
            spriteBatch.Draw(TextureAssets.Npc[NPC.type].Value, drawPos2 + new Vector2(0, -6), NPC.frame, color2, NPC.rotation, drawOrigin2, NPC.scale, SpriteEffects.None, 0f);
            return false;
        }
        ...
        public void ExternalSwitchAttack(float attack)
        {
            NPC.GetGlobalNPC<GlobalEnemyBossInfo>().attack = attack;
            switch (attack)
            {
                case (float)ActionState.NormalJump:
                    AI_Timer = 0;
        ...
        {/*
            NPC.GetGlobalNPC<GlobalEnemyBossInfo>().attack = (Main.npc[(int)NPC.ai[0]].GetGlobalNPC<GlobalEnemyBossInfo>().attack + Main.rand.Next(1, 6)) % 6;
