...
namespace AwfulGarbageMod.NPCs
{
    ...
    public class RottingSoulflower : ModNPC
    {
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            ...
            bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("Mods.AwfulGarbageMod.Bestiary.RottingSoulflower")
            });
        }
        ...
        public override void AI()
        {
            ...
            switch (AI_State)
            {
                ...
                case (float)ActionState.Teleport2:
                    Teleport2();
                    break;
                case (float)ActionState.Ballohurt:
                    Ballohurt();
                    break;
                ...
                case (float)ActionState.Teleport4:
                    Teleport4();
                    break;
                case (float)ActionState.Vilethorn:
                    Vilethorn();
                    break;
            }
        }
        ...
        private void Teleport2()
        {
            ...
            if (AI_Timer == 0)
            {
                NPC.alpha = 0;
                AI_Timer = 0;
                AI_State = (float)ActionState.Ballohurt;
            }
        }
        ...
        private void Teleport4()
        {
            ...
            if (AI_Timer == 0)
            {
                NPC.alpha = 0;
                AI_Timer = 0;
                AI_State = (float)ActionState.Vilethorn;
            }
        }
        ...
        private void Shoot()
        {
            if (AI_Timer % 60 == 30)
            {
                int proj = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, NPC.velocity, ProjectileID.CursedFlameHostile, 8, 0, Main.myPlayer);
        ...
        private void Ballohurt()
        {
            ...
            {
                shotXspd = -17.5f;
                shotXspd += Main.rand.NextFloat(-1.5f, 1.5f);
            }
            if (AI_Timer > 30 && AI_Timer <= 45)
            {
                int proj = Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, NPC.velocity, ModContent.ProjectileType<HostileBoHProj>(), 10, 0, Main.myPlayer, NPC.whoAmI);
                Main.projectile[proj].velocity = new Vector2(shotXspd, -12);
                shotXspd += 2.5f;
            }
        ...
        private void Summon()
        {
            if (AI_Timer % 120 == 60)
            {
               int nPC = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, NPCID.EaterofSouls);
        ...
        private void Vilethorn()
        {
            if (AI_Timer == 30)
            {
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(160, 0), ModContent.ProjectileType<VilethornSpawner>(), 0, 0, Main.myPlayer);
                Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(-160, 0), ModContent.ProjectileType<VilethornSpawner>(), 0, 0, Main.myPlayer);
