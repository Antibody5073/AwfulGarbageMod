...
namespace AwfulGarbageMod.Items.Weapons.Magic
{
    ...
    public class OcularScepter : ModItem
	{
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("OcularScepterOrbit").Type] < ModItemSets.Sets.MaxScepterProjectiles[Item.type] + player.GetModPlayer<GlobalPlayer>().MaxScepterBoost)
            {
