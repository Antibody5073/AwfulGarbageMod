...
namespace AwfulGarbageMod.Items.Weapons.Melee
{
    public class Leafstorm : ModItem
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.damage = 37; // The damage of your flail, this is dynamically adjusted in the projectile code.
            Item.noUseGraphic = true; // This makes sure the item does not get shown when the player swings his hand
            Item.shoot = ModContent.ProjectileType<LeafstormProj>(); // The flail projectile
    ...
    public class LeafstormProj : BaseFlailProjectile
    {
        public override string ChainTexturePath()
        {
            return "AwfulGarbageMod/Items/Weapons/Melee/LeafstormProjectileChain"; 
        }
        public override void SetStaticDefaults()
        {
        ...
        public override float MaxSpinDistance => 90;
        public override float SpinSpd => 0.6f;
        ...
        public override int spinHitCooldown => 120;
        public override float RetractSpd => 0.2f;
        public override float MaxRetractSpd => 7f;
        public override float flailHeadRotationRetract => 180;
        public override float flailHeadRotation => 90;


        public override void RetractEffect(Player player, float length, float maxLength)
        {
            if (length == maxLength)
            {
            Vector2 vel = Projectile.Center - player.Center;
                for (var i = 0; i < 3; i++)
                {
                    int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, vel.SafeNormalize(Vector2.Zero) * 10 + Main.rand.NextVector2Circular(2, 2), ProjectileID.BladeOfGrass, Projectile.damage / 2, 0, Projectile.owner);
                }
            }
        }
        ...
        public override void SpinEffect(Player player, float timer, float length, float maxLength)
        {
            if (length == maxLength)
            {
                ...
                {
                    int dust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.GrassBlades, 0f, 0f, 0, default(Color), 1f);
        ...
        public override void HitEffect(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (var i = 0; i < 2; i++)
            {
                int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, new Vector2(0, 5).RotatedByRandom(MathHelper.ToRadians(360)), ProjectileID.BladeOfGrass, Projectile.damage / 5, 0, Projectile.owner);
            }
        }
