...
namespace AwfulGarbageMod.Items.Weapons.Ranged
{
    ...
    public class EctoDaggers : ModItem
	{
		...
		public override void SetDefaults()
		{
			...
			Item.autoReuse = true;
			Item.shoot = ModContent.ProjectileType<EctoDaggersProjDummy>();
            Item.shootSpeed = 9f;
        }
    ...
    public class EctoDaggersProjDummy : ModProjectile
    {
        public override void SetDefaults()
        {
        ...
        public override void OnKill(int timeLeft)
        {
            for (var i = 0; i < 3; i++)
            {
                int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position, Projectile.velocity.RotatedBy(MathHelper.ToRadians(Main.rand.NextFloat(-8, 8))) * Main.rand.NextFloat(0.9f, 1.1f), ModContent.ProjectileType<EctoDaggersProj>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
    ...
    public class EctoDaggersProj : ModProjectile
    {
        public override void SetStaticDefaults()
        {
        ...
        public override void OnKill(int timeLeft)
        {
            for (var i = 0; i < 20; i++)
            {
                Vector2 dustvel = new Vector2(Main.rand.NextFloat(4, 14.5f), 0).RotatedBy(Projectile.velocity.ToRotation() + MathHelper.ToRadians(Main.rand.NextFloat(-18, 18)));
                int dust = Dust.NewDust(Projectile.position, 1, 1, DustID.SpectreStaff, dustvel.X, dustvel.Y, 0, default(Color), 1f);
