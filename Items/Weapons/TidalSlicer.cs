        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
			...
			if (counter % 2 == 1)
			{
				if (Main.rand.Next(5) < 1)
				{
					for (var i = -1; i < 2; i++)
					{
						int proj = Projectile.NewProjectile(Projectile.GetSource_NaturalSpawn(), position, velocity.RotatedBy(MathHelper.ToRadians(10*i)), Mod.Find<ModProjectile>("WaterStreamMelee").Type, damage, knockback, player.whoAmI);
				...
				{
                    int proj = Projectile.NewProjectile(Projectile.GetSource_NaturalSpawn(), position, velocity, Mod.Find<ModProjectile>("WaterStreamMelee").Type, damage, knockback, player.whoAmI);
                    Main.projectile[proj].CritChance = Item.crit + (int)player.GetCritChance(DamageClass.Melee) + (int)player.GetCritChance(DamageClass.Generic) + 4;
                }
            }
			return false;
