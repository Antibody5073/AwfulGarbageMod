...
namespace AwfulGarbageMod.Items.Weapons.Summon
{
    ...
    public class PileOfActualBonesMinionBuff : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            if (player.ownedProjectileCounts[ModContent.ProjectileType<PileOfActualBonesMinion>()] > 0)
            {
    ...
    public class PileOfActualBones : ModItem
	{
		public override void SetStaticDefaults()
		{
		...
		public override void SetDefaults()
		{
            ...
            Item.buffType = ModContent.BuffType<PileOfActualBonesMinionBuff>();
            // No buffTime because otherwise the item tooltip would say something like "1 minute duration"
            Item.shoot = ModContent.ProjectileType<PileOfActualBonesMinion>(); // This item creates the minion projectile
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (player.ownedProjectileCounts[Mod.Find<ModProjectile>("PileOfActualBonesMinion").Type] > 0)
            {
                for (int i = 0; i < Main.maxProjectiles; i++)
                {
                    Projectile projectile = Main.projectile[i];
                    if (projectile.type == Mod.Find<ModProjectile>("PileOfActualBonesMinion").Type && projectile.owner == player.whoAmI)
                    {
                        projectile.originalDamage += (int)(Item.damage * 0.67f);
    ...
    public class PileOfActualBonesMinion : ModProjectile
    {
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<PileOfActualBonesMinionBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<PileOfActualBonesMinionBuff>()))
            {
        ...
        private void Movement(bool foundTarget, float distanceFromTarget, Vector2 targetCenter, NPC target, float distanceToIdlePosition, Vector2 vectorToIdlePosition)
        {
            ...
            if (foundTarget)
            {
                ...
                if (atkTimer % 33 == 0)
                {
                    ...
                    direction.Normalize();
                    int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, direction * 14f, Mod.Find<ModProjectile>("PileOfActualBonesProjectile").Type, Projectile.damage, Projectile.knockBack, Projectile.owner);
                }
                ...
                if (lineOfSight)
                {
                    speed = Vector2.Distance(Projectile.Center, targetCenter) / 36f - 5f;
                }
                else
                {
                    speed = Vector2.Distance(Projectile.Center, targetCenter) / 36f;
                }
                if (speed > 8)
                {
                    speed = 8;
    ...
    public class PileOfActualBonesProjectile : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Crossbone"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display name here by uncommenting this line.
